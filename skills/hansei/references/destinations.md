...
Every probe answers two questions, not one: does this destination exist here, and does it
already contain this exact fix? The second question is the one that kills duplicates. A
`scripts/` directory existing does not mean your script is missing; look for the script itself.
An `.env.example` existing does not mean your key is absent; grep for it.
